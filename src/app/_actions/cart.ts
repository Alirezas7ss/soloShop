...
export async function addToCartAction(input: CartItem) {
  ...
  if (!cartId) {
    const cart = await db.reservation.create({
      data: {
        items: [input],
      },
    })
  ...
  const cart = await db.reservation.findFirst({
    where: {
      id: cartId,
    },
  });
  ...
  const cartItem = cart.items?.find((item) => item.id === input.productId)
  ...
  if (input.quantity > 0) {
    if (cartItem) {
      ...
      cart.listing?.push(input)
    }
  }
  // If the quantity is 0, remove the item from the cart
  else if (cartItem) {
    cart.listing =
      cart.listing?.filter((item) => item.id !== input.productId) ?? []
  }
